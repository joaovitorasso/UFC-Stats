...
def baixar_html(cfg: PipelineConfig, fighters_index_path: Path, *, dt: str, run_id: str, use_cache: bool = True) -> Path:
    ...
    cache_hits = sum(
        ...
        for f in lutadores
        if use_cache and (
            (out_dir / f"{f.get('fighter_id') or fighter_id_from_url(f.get('profile_url'))}.html").exists()
    ...
    def _download_lutador(args: tuple[int, dict]) -> tuple[str, bool]:
        ...
        if use_cache and html_path.exists():
            return fid, True
        if use_cache and fid in ids_em_cache_anterior:
            return fid, True
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for future in as_completed(futures):
            ...
            if a_baixar > 0 and (count % 100 == 0 or count == a_baixar):
                print(f"  [progresso] {count}/{a_baixar} lutadores baixados", flush=True)
...
baixar_html_lutadores = baixar_html
...
# Alias para compatibilidade com o código legado
gerar_bronze_lutadores = gerar_bronze
