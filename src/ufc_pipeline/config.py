...
from ufc_pipeline.http import HttpConfig
...
def load_config(repo_root: Path) -> tuple[PipelineConfig, Dict[str, Any]]:
    ...
    cfg = PipelineConfig(
        data_dir=data_dir,
        http=http,
        pipeline_name=str(pipe_cfg.get("pipeline_name", "ufc_lakehouse")),
        events_lookback_days=int(pipe_cfg.get("events_lookback_days", 30)),
        full_load_if_db_empty=_to_bool(pipe_cfg.get("full_load_if_db_empty"), True),
