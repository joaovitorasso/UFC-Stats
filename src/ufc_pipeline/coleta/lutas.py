...
from ufc_pipeline.ids import fight_id, fighter_id_from_url, short_id_from_id
from ufc_pipeline.io import read_jsonl, read_text, write_text, write_jsonl, write_json, ensure_dir
from ufc_pipeline.parsing import clean, safe_attr
...
def _parse_links_lutas(event_html: str) -> tuple[str | None, list[dict]]:
    ...
    for row in soup.select("tr.b-fight-details__table-row"):
        ...
        if not link:
            continue
        lutas.append({"fight_url": str(link), "bonus_code": _codigo_bonus(row), "bout": _tipo_bout(row)})
...
def _parse_golpes_por_round(soup: BeautifulSoup) -> dict:
    sig_title_p = soup.find(
        "p", class_="b-fight-details__collapse-link_tot",
...
def _download_fight_html(
    client: HttpClient,
    fight_url: str,
    fight_cache: Path,
    label: str,
) -> tuple[str, bool]:
    """Baixa (ou lê do cache) o HTML de uma luta. Retorna (html, from_cache)."""
...
def gerar_bronze(
    ...
    *,
    ...
    # ── Fase 1: montar fila de tarefas ────────────────────────────────────────
    # Cada tarefa: (fight_url, fight_cache, metadados para o parse)
    tarefas: list[dict] = []
    for ev_idx, ev in enumerate(eventos, start=1):
        ...
        if not html_path.exists():
            log.warning("[BRONZE/lutas] HTML não encontrado, pulando evento %s", eid)
            continue
        ...
        for idx, row in enumerate(fight_rows, start=1):
            ...
            tarefas.append({
                ...
                "was_cached": fight_cache.exists(),  # True = já baixado em run anterior
    ...
    htmls: dict[str, str] = {}   # fight_url -> html

    def _worker(tarefa: dict) -> tuple[str, str]:
        html, _ = _download_fight_html(
            client, tarefa["fight_url"], tarefa["fight_cache"], tarefa["label"]
        )
    ...
    for t in tarefas:
        ...
        # Só indexar lutadores de lutas NOVAS (não cacheadas).
        # Lutas já cacheadas tiveram seus lutadores indexados em runs anteriores.
        if not t["was_cached"]:
    ...
    log.info("[BRONZE/lutas] Total: %d lutas parseadas, %d lutadores únicos", len(todas_lutas), len(index_lutadores))
    ...
    index_dir = cfg.data_dir / "bronze" / "lutadores_index" / f"dt={dt}"
    ...
    write_json(meta_path, {
        "run_id": run_id, "dt": dt, "table": "bronze_lutas",
        "rows": len(todas_lutas), "created_at": ingested_at,
    })
