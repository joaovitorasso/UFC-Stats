...
from ufc_pipeline.ids import event_id_from_url, short_id_from_id
from ufc_pipeline.io import write_jsonl, write_json, read_jsonl, write_text, ensure_dir
from ufc_pipeline.parsing import clean, safe_select_one, safe_attr, parse_ufc_date
from ufc_pipeline.banco import ConexaoBanco, teve_sucesso_anterior, banco_tem_dados
...
def coletar_index(cfg: PipelineConfig, sources: dict, *, dt: str, run_id: str) -> Path:
    client = HttpClient(cfg.http)
    ufc = sources["ufcstats"]
    data_ancora = date.fromisoformat(dt)

    carga_completa = False
    if cfg.full_load_if_db_empty:
        banco = ConexaoBanco.do_env()
        state = teve_sucesso_anterior(banco, cfg.pipeline_name)
        if state is False:
            carga_completa = True
        elif state is None:
            if banco_tem_dados(banco) is False:
                carga_completa = True
    ...
    write_json(meta_path, {
        "run_id": run_id, "dt": dt, "stage": "raw_events_index",
        "rows": len(eventos), "created_at": datetime.utcnow().isoformat() + "Z",
    })
    return out_path


def baixar_html(cfg: PipelineConfig, events_index_path: Path, *, dt: str, run_id: str) -> Path:
    client = HttpClient(cfg.http)
    ...
    total = len(eventos)
    cache_hits = sum(1 for ev in eventos if (out_dir / f"{ev.get('event_id') or event_id_from_url(ev.get('event_url'))}.html").exists())
    ...
    log.info("[RAW/html] %d HTMLs de eventos baixados, %d já existiam (pulados).", count, skipped)

    meta_path = cfg.data_dir / "raw" / "_meta" / "runs" / f"{run_id}_event_html.json"
    write_json(meta_path, {
        "run_id": run_id, "dt": dt, "stage": "raw_event_html",
...
def gerar_bronze(cfg: PipelineConfig, events_index_path: Path, *, dt: str, run_id: str) -> Path:
    ...
    write_json(meta_path, {
        "run_id": run_id, "dt": dt, "table": "bronze_eventos",
        "rows": len(eventos), "created_at": ingested_at,
    })
    return out_path
